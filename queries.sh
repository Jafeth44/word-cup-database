...
echo "$($PSQL "select name from teams full join games on teams.team_id = games.winner_id WHERE round = 'Eighth-Final' AND year = 2014 UNION SELECT name from teams full join games on teams.team_id = games.opponent_id WHERE round
...
echo "$($PSQL "SELECT b.year, a.name FROM teams a INNER JOIN games b ON a.team_id = b.winner_id WHERE round = 'Final' ORDER BY year")"
